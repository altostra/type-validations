 ...
 * @param rejectionGenerator A function that can create rejection reason for a failed value
 * @param type A string that describe the validated type
 * @returns A type validator that wraps a validator function with function that emit rejection
 * reasons mapped from the original rejections
 */
...
export function setRejectReasonsColoring(isColored: boolean): void {
	colors = isColored
}
